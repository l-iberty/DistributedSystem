...
{
    RaftNode* rfNode = static_cast<RaftNode*>(param);
    rfNode->runRpcServer();
    return nullptr;
}
...
RaftNode::RaftNode(int id, std::vector<uint16_t> rpcServerPorts, std::vector<uint16_t> userPorts)
        : mId(id),
          mState(Follower),
          mRpcServerPorts(rpcServerPorts),
          mUserPorts(userPorts),
          mCurrentTerm(INVALID_TERM),
          mVotedFor(INVALID_ID),
          mCommitIndex(INVALID_INDEX),
          mLastApplied(INVALID_INDEX),
          mNextIndex(rpcServerPorts.size(), 0),
          mMatchIndex(rpcServerPorts.size(), INVALID_INDEX),
          mCurrentLeaderId(INVALID_ID),
          mTCPServer(userPorts[id])
{
    ...
    if (pthread_mutex_init(&mLoglistMutex, nullptr) < 0)
    {
        perror(STDOUT_RED "#ERROR# pthread_mutex_init" STDOUT_NONE);
    }
}
...
uint16_t RaftNode::rpcServerPort()
{
    return mRpcServerPorts[mId];
}

std::vector<uint16_t> RaftNode::serverPortsOfAllNodes()
{
    return mRpcServerPorts;
}
...
std::vector<Log> RaftNode::sliceLoglist(int start, int end)
{
    ...
    end = std::min(end, static_cast<int>(mLoglist.size() - 1));
...
void RaftNode::run()
{
    ...
    pthread_t user = createUserThread();

    pthread_join(server, &retval);
    pthread_join(client, &retval);
    pthread_join(user, &retval);
}
...
// 接收并处理用户的请求. 如果节点的当前状态是Leader则直接处理, 否则将用户请求重定向到Leader
void RaftNode::runUserRequestHandler()
{
    /* 开始监听 */
    if (!mTCPServer.listen()) return;

    for (;;)
    {
        /* 接收客户端的连接请求 */
        if (!mTCPServer.accept()) return;

        char buf[1];
        if (mTCPServer.recvFromClient(buf, sizeof(buf)))
        {
            if (mState == Leader)
            {
                printf(STDOUT_L_GREEN "%s[%d] received a request, appending log entries...\n" STDOUT_NONE,
                       stateToString().c_str(), mId);
                appendLog(1);
            }
            else if (mCurrentLeaderId != INVALID_ID)
            {
                printf(STDOUT_L_GREEN "%s[%d] received a request, relocate it to Leader[%d]\n" STDOUT_NONE,
                       stateToString().c_str(), mId, mCurrentLeaderId);

                if (mTCPClient.connectServer("127.0.0.1", mUserPorts[mCurrentLeaderId]))
                {
                    if (!mTCPClient.sendToServer(buf, sizeof(buf)))
                    {
                        printf(STDOUT_RED "#ERROR# unable to relocate request to leader\n" STDOUT_NONE);
                    }
                }
                else
                {
                    printf(STDOUT_RED "#ERROR# unable to connect to leader\n" STDOUT_NONE);
                }
                mTCPClient.closeConnection();
            }
        }
        mTCPServer.closeConnection();
    }
}
...
std::vector<RequestVoteResult> RaftNode::doRequestVote()
{
    ...
    for (int id = 0; id < mRpcServerPorts.size(); id++)
    {
        if (id != mId)
        {
            RequestVoteResult res;
            uint16_t port = mRpcServerPorts[id];
...
std::vector<AppendEntriesResult> RaftNode::doAppendEntries()
{
    ...
    for (int id = 0; id < mRpcServerPorts.size(); id++)
    {
        if (id != mId)
        {
            ...
            std::vector<Log> entries = sliceLoglist(mNextIndex[id], mNextIndex[id] + BATCH_SIZE - 1);
        ...
        int term, int leaderId, int prevLogIndex, int prevLogTerm, int leaderCommit, std::vector<Log>& entries)
{
    ...
    if (term == mCurrentTerm)
    {
        ...
        pthread_mutex_lock(&mLoglistMutex);
        ...
        pthread_mutex_unlock(&mLoglistMutex);
        matchIndex = index;
    }
...
{
    pthread_t tid;
    if (pthread_create(&tid, nullptr, serverThread, this) != 0)
    {
...
{
    pthread_t tid;
    if (pthread_create(&tid, nullptr, clientThread, this) != 0)
    {
        perror(STDOUT_RED "#ERROR# pthread_create" STDOUT_NONE);
        exit(EXIT_FAILURE);
    }
...
pthread_t RaftNode::createUserThread()
{
    pthread_t tid;
    if (pthread_create(&tid, nullptr, userThread, this) != 0)
    {
        perror(STDOUT_RED "#ERROR# pthread_create" STDOUT_NONE);
        exit(EXIT_FAILURE);
    }
...
void RaftNode::appendLog(int n)
{
    Log e;
    e.id = static_cast<int>(mLoglist.size());
    e.term = mCurrentTerm;
